"""
Adapted from https://github.com/fopina/django-bulk-update-or-create
"""
...
class BulkUpdateOrCreateMixin:
    ...
    def __bulk_update_or_create(
        ...
        if not objs:
            raise ValueError("no objects to update or create.")
        if not update_fields:
            raise ValueError("fields to update cannot be empty")
                ...
                for field in match_field:
                    if not hasattr(obj, field):
                        raise AttributeError(
                            f"'{obj.__class__.__name__}' has no {field} field"
                        )
            else:
                if not hasattr(obj, match_field):
                    raise AttributeError(
                        f"'{obj.__class__.__name__}' has no {match_field} field"
                    )

            for _f in update_fields:
                if not hasattr(obj, _f):
                    raise AttributeError(f"some objects do not have the {_f} field")

        # get object in batches
        batches = (objs[i : i + batch_size] for i in range(0, len(objs), batch_size))

        # get value of match_field from obj
        if case_insensitive_match:

            def _cased_key(obj):
                if isinstance(match_field, list):
                    k = [getattr(obj, i) for i in match_field]
                    k = tuple(i.lower() if hasattr(i, "lower") else i for i in k)
                else:
                    k = getattr(obj, match_field)
                    k = k.lower() if hasattr(k, "lower") else k
                return k
            ...
            def _cased_key(obj):
                if isinstance(match_field, list):
                    return tuple(getattr(obj, i) for i in match_field)
                return getattr(obj, match_field)

        for batch in batches:
            # create obj dictionary with match field value as key
            ...
            if isinstance(match_field, list):
                ...
                for item in obj_map.keys():
                    q_objects |= Q(**{j: item[i] for i, j in enumerate(match_field)})

                to_update = list(self.filter(q_objects))
            else:
                to_update = list(self.filter(**{match_field + "__in": obj_map.keys()}))

            for item in to_update:
                obj = obj_map[_cased_key(item)]
                # for field in update field
                for _f in update_fields:
                    setattr(item, _f, getattr(obj, _f))
                del obj_map[_cased_key(item)]

            self.bulk_update(to_update, update_fields)

            # .create on the remaining (NOTE: bulk_create won't work on multi-table inheritance models...)
            created_objs = []
...
class BulkUpdateOrCreateQuerySet(BulkUpdateOrCreateMixin, models.QuerySet):
    def order_by_null_last(self, *args, **kwargs):
        ordering = []
        for term in args:
            if term.startswith("-"):
                ordering.append(F(term[1:]).desc(nulls_last=True))
            else:
                ordering.append(F(term).asc(nulls_last=True))
