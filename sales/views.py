...
class DashboardGetCountView(generics.ListAPIView):
    def get(self, request, *args, **kwargs):
        ...
        highest_customer_serializer = CustomerReportSerializer(highest_customer)
        ...
        highest_employee_serializer = EmployeeReportSerializer(highest_employee)
        ...
        customers_without_purchase = Customer.objects.filter(sales__isnull=True).count()
        employees_without_sale = User.objects.filter(sales__isnull=True, sales_perm=True).count()
...
class DashboardCategoryChartView(generics.ListAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        categories = Category.objects.filter(items__isnull=False).annotate(total_quantity=Sum('items__quantity')).order_by('-total_quantity')

        first_five_categories = categories[:5]
        first_five_sum = first_five_categories.aggregate(total_sum=Sum('total_quantity'))['total_sum']
        total_sum = categories.aggregate(total_sum=Sum('total_quantity'))['total_sum']
        remaining_sum = total_sum - first_five_sum

        first_five_serializers = CategoryChartSerializer(first_five_categories, many=True)
        others = dict()
        others['name'] = 'others'
